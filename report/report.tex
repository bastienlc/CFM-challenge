...
\begin{multicols}{2}
    ...
    Overall, while we managed to achieve a $40\%$ accuracy on the test set, we are not satisfied with our results. We believe that the main reason for the low accuracy can be found in the distribution of the test set. While we understand the practical importance of having a test set from a different period of time, we believe that in that case it would be beneficial to have time series data so that trend changes can be detected and exploited on the test set.
